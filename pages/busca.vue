<template>
    <div class="container mx-auto py-8">
      ...
      <div class="mb-4 flex">
        ...
        <button @click="search" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md ml-4">Pesquisar</button>
      </div>
      <div v-if="loading">
        <p>Carregando...</p>
      </div>
      <div v-else>
        ...
        <div v-if="commonLanguages.length > 0">
          ...
          <ul>
            <li v-for="language in commonLanguages" :key="language" @click="filterByLanguage(language)" class="cursor-pointer hover:text-blue-500">{{ language }}</li>
          </ul>
        </div>
      </div>
      ...
      <div v-if="showModal" class="fixed inset-0 flex justify-center items-center">
        ...
        <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md relative z-10">
          ...
          <button @click="closeModal" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-md mt-4">Fechar</button>
        </div>
      </div>
    </div>
  </template>
