<template>
  <div class="bg-gray-100 min-h-screen flex justify-center items-center">
    <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md my-4">
      <h1 class="text-3xl font-semibold mb-6 text-center text-blue-600">Cadastro</h1>
      <form @submit.prevent="handleSubmit">
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Data de Nascimento</label>
          <input v-model="dob" type="date" class="form-input w-full border-gray-300 rounded-md" required :max="maxDate" :min="minDate">
        </div>
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Nome Completo</label>
          <input v-model.trim="fullName" type="text" class="form-input w-full border-gray-300 rounded-md" placeholder="Informe seu nome completo" required>
        </div>
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">CPF</label>
          <input v-model="cpf" type="text" class="form-input w-full border-gray-300 rounded-md" placeholder="Informe seu CPF" required @blur="validateCPF" v-mask="'###.###.###-##'">
        ...
        <div class="mb-4" v-if="petBreed === 'outro'">
          ...
          <input v-model.trim="otherBreed" type="text" class="form-input w-full border-gray-300 rounded-md" placeholder="Informe a raça do pet" required>
        </div>
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Renda Mensal</label>
          <div class="flex items-center">
            <span class="inline-block bg-gray-200 py-2 px-3 rounded-l-md border border-r-0 border-gray-300 text-gray-700">R$</span>
            <input v-model.number="monthlyIncome" type="number" class="form-input w-full border-gray-300 rounded-r-md" placeholder="Informe sua renda mensal" required min="1000">
          </div>
        </div>
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">CEP</label>
          <input v-model.trim="cep" type="text" class="form-input w-full border-gray-300 rounded-md" placeholder="Informe seu CEP" required @input="fetchAddress" v-mask="'#####-###'">
        </div>
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Rua</label>
          <input v-model.trim="street" type="text" class="form-input w-full border-gray-300 rounded-md" placeholder="Informe sua rua" required>
        </div>
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Bairro</label>
          <input v-model.trim="neighborhood" type="text" class="form-input w-full border-gray-300 rounded-md" placeholder="Informe seu bairro" required>
        </div>
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Cidade</label>
          <input v-model.trim="city" type="text" class="form-input w-full border-gray-300 rounded-md" placeholder="Informe sua cidade" required>
        </div>
        <div class="mb-4">
          <label class="block text-gray-700 mb-2">Estado</label>
          <input v-model.trim="state" type="text" class="form-input w-full border-gray-300 rounded-md" placeholder="Informe seu estado" required maxlength="2">
        </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-gray-600 bg-opacity-75 flex justify-center items-center">
      <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 class="text-2xl font-semibold mb-4 text-blue-600">Confirmação dos Dados</h2>
        ...
        <div class="flex justify-end mt-4">
          <button @click="closeModal" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-md mr-2">Fechar</button>
          <button @click="submitData" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">Confirmar</button>
        </div>
      </div>
    </div>
  </div>
